...
},{}],"js/detail_check.js":[function(require,module,exports) {
"use strict";

var _data = _interopRequireDefault(require("./data.js"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
/* 색상 선택 */
var sortColor = document.querySelectorAll('.detail-info .color div');
var miniView = document.querySelectorAll('.detail-view .img-small div img');

/* 초기화 */
sortColor[0].style.border = '3px solid gray';
sortColor.forEach(function (cloth) {
  cloth.addEventListener('click', function () {
    var color = this.getAttribute('data-color');
    var srcs;
    for (var i = 0; i < _data.default.length; i++) {
      if (_data.default[i].name == 'detail' && _data.default[i].color == color) {
        srcs = _data.default[i].src;
        break;
      }
    }
    var thumbnail = document.querySelector('.detail-view .img-thumb img');
    thumbnail.setAttribute('src', srcs[0]);
    miniView.forEach(function (value, index) {
      value.setAttribute('src', srcs[index]);
    });

    /* 클릭하면 border 강조 */

    sortColor.forEach(function (value) {
      return value.style.border = '3px solid #eee';
    });
    this.style.border = '3px solid gray';
  });
});
var sizeBtns = document.querySelectorAll('.size div');
sizeBtns.forEach(function (btn) {
  btn.addEventListener('click', function () {
    sizeBtns.forEach(function (btn) {
      return btn.style.border = '3px solid #eee';
    });
    btn.style.border = '3px solid gray';
  });
});
var thumbnailSmall = document.querySelectorAll('.detail-view .img-small div img');
var thumbnail = document.querySelector('.detail-view .img-thumb img');
thumbnailSmall.forEach(function (img) {
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/detail_check.js"], null)
//# sourceMappingURL=/detail_check.ef3afb23.js.map