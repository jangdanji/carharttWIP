...
function filtering() {
  ...
  var products = [];
  var checkList = [];
  var AllFilter = document.querySelectorAll('.side-filter label input');
  AllFilter.forEach(function (filter) {
    if (filter.checked) checkList.push(filter.getAttribute('data'));
  });
  var clothChecked = checkList.filter(function (check) {
    return check.includes('cloth');
  });
  var priceChecked = checkList.filter(function (check) {
    return check.includes('price');
  });
  var colorChecked = checkList.filter(function (check) {
    return check.includes('color');
  });
  var productData = _data.default.filter(function (data) {
    return data.name == 'product-list';
  });
  productData.forEach(function (value) {
    var clothCheck; /* 카테고리 분류 */
    var colorCheck; /* 색상 분류 */
    var priceCheck; /* 가격 분류 */

    if (clothChecked.length == 0) clothCheck = true; /* 옷 카테고리 */else clothCheck = clothChecked.some(function (cloth) {
      return value.category == cloth;
    });
    if (colorChecked.length == 0) colorCheck = true; /* 색상 */else colorCheck = colorChecked.some(function (color) {
      return value.color == color;
    });
    if (priceChecked.length == 0) priceCheck = true; /* 가격 */else {
      priceCheck = priceChecked.some(function (priceIdx) {
        var price = value.price;
        priceIdx = priceIdx.replace('price-', '');
        priceIdx = parseInt(priceIdx);
        var priceFilters = [price <= 50000, 50000 <= price && price <= 150000, 150000 <= price && price <= 250000, 250000 <= price && price <= 500000, 500000 < price];
...
function get12(index, products) {
  ...
  for (var _i = indexStart; _i <= indexEnd; _i++) {
    ...
    product_box.innerHTML = "\n            <div class=\"imgBox\">\n                <a href=\"./detail.html\">\n                    <img src=".concat(products[_i].src[0], " alt=\"product-image\">\n                    <img src=").concat(products[_i].src[1], " alt=\"product-image\">\n                </a>\n            </div>\n            \n            <div class=\"product-info\">\n                <div class=\"product-status\">\n                    <span></span>\n                    <span></span>\n                </div>\n                <div class=\"product-text\">\n                    <p class=\"product-name-kr\">").concat(products[_i].productKR, "</p>\n                    <p class=\"product-name-eng\">").concat(products[_i].product, "</p>\n                    <p class=\"product-price\">&#8361; ").concat(comma(products[_i].price), "</p>\n                </div>\n                <div class=\"product-buy\">\n                    <span><i class=\"fas fa-shopping-cart\"></i></span>\n                    <span><i class=\"fas fa-heart\"></i></span>\n                </div>\n            </div>\n        ");
    /* 세번째 박스는 margin right 0 */
    /* 2, 5, 7 */
    if ((_i + 1) % 3 == 0) product_box.style.marginRight = '0';
...
var colorCheckIcon = document.querySelectorAll('.side-filter .filter-color label i.fa-check');
filterReset.addEventListener('click', function () {
  AllFilter.forEach(function (checkbox) {
    return checkbox.checked = false;
  });
  colorCheckIcon.forEach(function (chkIcon) {
    return chkIcon.style.display = 'none';
  }); /* 이건 동적인 요소라서 수동으로 ... */

  var products = _data.default.filter(function (data) {
    return data.name == 'product-list';
  });
  removeBox();
  get12(0, products);
